...
object Redis {
  trait Service[R] {
    ...
    def rename[A: Write, B: Write](oldKey: A, newKey: B): ZIO[R, Nothing, Unit]
    ...
    def set[A: Write, B: Write](key: A, value: B): ZIO[R, Nothing, Unit]
  }
  ...
  object > extends Service[Redis] {
    ...
    def rename[A: Write, B: Write](oldKey: A, newKey: B): ZIO[Redis, Nothing, Unit] = ZIO.accessM[Redis](_.redis.rename(oldKey, newKey))
    ...
    def ping: ZIO[Redis, Nothing, Unit] = ZIO.accessM[Redis](_.redis.ping)
    def get[A: Write](key: A): OptionalResult[Redis] = OptionalResult(ZIO.accessM[Redis](_.redis.get(key).bytes))
    def set[A: Write, B: Write](key: A, value: B): ZIO[Redis, Nothing, Unit] = ZIO.accessM[Redis](_.redis.set(key, value))
  }

  case class OptionalResult[R](bytes: ZIO[R, Nothing, Option[Chunk[Byte]]]) {
    def unit: ZIO[R, Nothing, Unit] = bytes.unit
    def as[A: Read]: ZIO[R, SerializationFailure, Option[A]] = bytes.flatMap {
      case Some(chunk) =>
        Read(chunk) match {
